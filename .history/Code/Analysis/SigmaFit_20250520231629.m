...
load("Analysis\ModelGen.mat", "mdl", "stim", "plain");
    ...
    for i = 1:length(select_fcns)
        ...
        SFit(base_dir, diameters, sigma_values, 'sigma_corr_matrix_focused.mat');
    end



function SFit(base_dir, diameters,  sigma_values,save_path)
    ...
    % Initialize storage for maxima across all heatmaps
    maxima_matrix = []; % Columns: [Diameter, Max_k, Max_sigma]
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            % Store the heatmap data for the current file
            heatmap_data(:, :, i) = heatmap_accumulator;
            % Increment file counter
            file_count = file_count + 1;

            % Find the global maxima for the current heatmap
            [max_val, max_idx] = max(heatmap_accumulator, [], 'all', 'linear');
            [max_k_idx, max_sigma_idx] = ind2sub(size(heatmap_accumulator), max_idx);

            % Store the maxima in the matrix
            diameter_value = str2double(diameter(1:end-2)); % Convert diameter to numeric
            maxima_matrix = [maxima_matrix; diameter_value, k_values(max_k_idx), sigma_values(max_sigma_idx), max_val];
        end
        close(progress);
    ...
    % Save the maxima data
    save('maxima_data_Diam.mat', 'maxima_matrix');
    disp('Maxima data saved to maxima_data_Diam.mat');
