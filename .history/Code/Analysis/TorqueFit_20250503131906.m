...
if sigma_corr
    ...
    centre = 1.8;
    k_const = 4;
    sigma_values = linspace(0.68, 0.78, 2);
    smooth_coeff = 50;
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    [~, existing_indices] = ismember(existing_sigma_values, sigma_values);
    disp([sigma_values, existing_sigma_values,existing_indices]);
    % Initialize the updated correlation matrix
    updated_correlation_matrix = zeros(length(diameters), length(sigma_values));
    ...
    existing_sigma_values = sigma_values;
    ...
    hold on;
    for d = 1:length(diameters)
        plot(sigma_values, correlation_matrix(d, :), 'DisplayName', sprintf('%s Diameter', diameters{d}));
    end
    hold off;
    xlabel('Sigma');
    ylabel('Correlation');
