...
EIT_O = EIT;
EIT = smoothdata(EIT(30:180,2:end), 'gaussian', 9);
...
closest_values = [13, 42,109,139]; % Adjusted for the EIT data range
...
for i = 1:length(closest_values)
    ...
    plot(calculate_envelope(abs(EIT(closest_values(i), :)-hom),20) * max(abs(EIT(closest_values(i), :)-hom)), 'DisplayName', sprintf("Slice %d", i));
end
...
function env = calculate_envelope(data, smooth_coeff)
    ...
    [env_ext, ~] = envelope(data_ext, 15, 'peak');
