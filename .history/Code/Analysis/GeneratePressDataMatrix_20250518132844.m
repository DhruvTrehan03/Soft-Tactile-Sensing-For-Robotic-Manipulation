...
centre = 1.8
k_values = linspace(0.01, 4.4, 100); % Define k values for the modulation
sigma_values = linspace(0.01, 4.4, 100);

% select_fcns = @(y,s,k) 0.5 * (y > k) - 0.5 * (y <= k) + (s*0); %step
% select_fcns = @(y, s, k) (y - k) / s; %linear
% select_fcns = @( y, s,k) exp(-(y - centre).^2 / (2 * s^2)) .* cos(k * (y - centre)),  % Modulated Gaussian
% select_fcns = @( y, s,k) -(y - k) .* exp(-((y - k).^2) / (2 * s^2)) / (s^2);     % Difference of Gaussians
% select_fcns = @(y,s,k)(y>centre-k/2).*(y<centre+k/2).*(y-centre)/s; %linear with k
% select_fcns = @(y, s) (y <= centre) .* exp(-(y - centre).^2 / (2 * s^2)) + (y > centre) .* exp(-(y - centre).^2 / (2 * s^2)) .* cos(k_const * (y - centre));% Modulated Gaussian with step
...
function sim_data_matrix = GenerateDataMatrix(mdl, stim, plain,plain_data, select_fcn, k_values, sigma_values)
    ...
    save('sim_data_matrix_MG.mat', 'sim_data_matrix', 'k_values', 'sigma_values');
...
function VisualizeSimDataMatrix(sim_data_matrix, k_values, sigma_values)
    ...
    figure;
    len = min(15, length(k_values)); % Set grid size to a maximum of 5x5 or less
