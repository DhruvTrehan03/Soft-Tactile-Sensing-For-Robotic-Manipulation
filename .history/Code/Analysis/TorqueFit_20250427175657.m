...
if sigma_corr
    ...
    for i = 1:length(select_fcns)
        ...
        fit_and_visualize(base_dir, diameters, mdl, stim, select_fcns{i}, sigma_values, smooth_coeff);
    end
end
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff)
    % FIT_AND_VISUALIZE: Fits data with simulated data using a given select_fcn
    % and visualizes the goodness of fit for all diameters on one plot with a slider.

    colors = lines(length(diameters)); % Generate distinct colors for plots
    figure; % Create a single figure for all diameters
    hold on;

    % Initialize storage for plots and data
    plot_handles = gobjects(length(diameters), 1);
    avg_corrs = cell(length(diameters), 1);
    ...
    for d = 1:length(diameters)
        ...
        % Compute average correlation for each sigma
        avg_corr = mean(correlations, 1);
        avg_corrs{d} = avg_corr; % Store for dynamic smoothing
        smoothed_corr = avg_corr; % Initial smoothed correlation

        % Plot results for this diameter
        plot_handles(d) = plot(sigma_values, smoothed_corr, 'Color', colors(d, :), 'LineWidth', 2, ...
            'DisplayName', sprintf('%s Diameter', diameter));
    end
    ...
    xlabel('Sigma');
    ylabel('Correlation');
    title('Correlation vs Sigma for All Diameters');
    legend('Location', 'best');
    grid on;

    % Add slider for smoothing factor
    uicontrol('Style', 'slider', ...
        'Min', 0, 'Max', 1, 'Value', 0, ...
        'Position', [20, 20, 200, 20], ...
        'Callback', @(src, event) update_smoothing(src.Value, avg_corrs, sigma_values, plot_handles));
...
function Corr_vs_K(data_dir, torque_values, mdl, stim, sigma_fixed, k_values, smooth_coeff)
    % CORR_VS_K: Computes and plots correlation against k for a fixed sigma.
    ...
    for i = 1:length(files)
        ...
        for j = 1:length(k_values)
            k = k_values(j);
            centre = 1.8;
            select_fcn = @(x, y, z) exp(-(y - centre).^2 / (2 * sigma_fixed^2)) .* cos(k * (y - centre));
    ...
    % Plot correlation vs k for each torque value
    figure;
    hold on;
    for i = 1:length(files)
        plot(k_values, correlations(i, :), 'DisplayName', sprintf('Torque %.2f', torque_values(i)));
    end
    hold off;
    xlabel('k');
    ylabel('Correlation');
...
function Corr_vs_K_Sigma(data_dir, torque_values, mdl, stim, k_values, sigma_values, smooth_coeff)
    % CORR_VS_K_SIGMA: Computes and plots correlation for a range of k and sigma values.
    ...
    % Initialize storage for results
    correlations = zeros(length(files), length(k_values), length(sigma_values));
    ...
    for i = 1:length(files)
        ...
        for j = 1:length(k_values)
            for k = 1:length(sigma_values)
                ...
                centre = 1.8;
                select_fcn = @(x, y, z) exp(-(y - centre).^2 / (2 * sigma^2)) .* cos(k_val * (y - centre));
    ...
    % Compute average correlation for each (k, sigma) pair
    avg_corr = squeeze(mean(correlations, 1));

    % Create heatmap
    figure;
    imagesc(k_values, sigma_values, avg_corr');
    colorbar;
    xlabel('k');
    ylabel('Sigma');
    title('Correlation vs k and Sigma');
    axis xy;
    grid on;
end

function [best_shift, max_corr] = find_best_shift_envelope(data_diff, sim_diff, shift_step, max_shifts, smooth_coeff)
    % Function to find the best shift using envelope correlation
