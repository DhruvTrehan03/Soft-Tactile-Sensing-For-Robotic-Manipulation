%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
load('C:\Users\dhruv\Documents\Tripos\4thYrProject\Readings\2024-12-05_18-15\device2.mat')
load('C:\Users\dhruv\Documents\Tripos\4thYrProject\Readings\2024-12-05_18-15\device3.mat')
...
[sorted_scores,sorted_indices] = analyze_channels(EIT_Interp,Torque_Interp,Time, 'fft');
sorted_EIT_Data = EIT_Interp(:,sorted_indices(1:169));
beta = (sorted_EIT_Data' * sorted_EIT_Data)\(sorted_EIT_Data' * Torque_Interp);
EIT_Torque = sorted_EIT_Data*beta;
figure();
plotLine(EIT_Torque,Time)
save("topChannels_fft.mat", "sorted_indices")
...
function [Data, Time, retained_indices] = preprocess(Raw_Data, burn_in, end_proportion)
    ...
    Data = Data - mean(Data, 1); % Center each channel
    Data = Data ./ std(Data, 0, 1); % Normalize each channel
end
...
function [sorted_scores,sorted_indices] = analyze_channels(eit_data, torque_data,time, method) % Choose from: 'correlation', 'snr', 'rms', 'fft', 'dtw'
    ...
    switch method
        ...
        case 'fft'
            scores = compute_fft_similarity(eit_data, torque_data);
        case 'dtw'
