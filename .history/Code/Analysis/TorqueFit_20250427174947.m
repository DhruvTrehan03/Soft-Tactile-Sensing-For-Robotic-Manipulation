...
if sigma_corr
    ...
    select_fcns = {
        @(y, s, k) exp(-(y - centre).^2 / (2 * s^2)) .* cos(k * (y - centre)),  % Modulated Gaussian
        @(y, s, k) -(y - centre) .* exp(-((y - centre).^2) / (2 * s^2)) / (s^2), % Difference of Gaussians
        @(y, s, k) (y > centre - k / 2) .* (y < centre + k / 2) .* (y - centre) / s, % Linear with k
        @(y, s, k) (y <= centre) .* exp(-(y - centre).^2 / (2 * s^2)) + ...
                   (y > centre) .* exp(-(y - centre).^2 / (2 * s^2)) .* cos(k * (y - centre)) % Modulated Gaussian with step
    };
