...
function envelope = calculate_envelope(data, smooth_coeff)
    ...
    [env_ext, ~] = envelope(data_ext, smooth_coeff, 'peak');

    % Extract only the middle section to avoid edge artifacts
    N = length(data);
    envelope = env_ext(N+1:2*N);

    % Smooth the envelope
    windowSize = 5;
    b = (1/windowSize) * ones(1, windowSize);
    a = 1;
    envelope = filter(b, a, envelope);

    % Normalize the envelope
    envelope = (envelope - min(envelope)) / (max(envelope) - min(envelope));
end
