...
config.sigma = 0.3;
config.k = 5;
config.select_fcn_type = 'mod_gauss'; % Options: 'mod_gauss', 'diff_gauss', 'step', 'linear', etc.
...
if sigma_corr
    analyze_correlation(config);
end

% Fit and Evaluate Torque
if fit
    fit_and_evaluate_torque();
end
...
function initialize_eidors()
    ...
    config.stim = create_stimulation(config.mdl); % Pass config.mdl to create_stimulation
end
...
function stim = create_stimulation(mdl)
    % Create stimulation patterns
    stim = mk_stim_patterns(32, 1, [0, 16], [0, 1], {'no_meas_current'}, 5);
    plain = mk_image(mdl, 1, 'Hi'); % Use mdl passed as an argument
...
function test_model(data_diff, sim_diff,config)
...
function select_fcn = get_select_fcn(config)
    % Return the appropriate select function based on the global configuration
    config;
    switch config.select_fcn_type
        case 'mod_gauss'
            select_fcn = @(x, y, z) exp(-(y - config.centre).^2 / (2 * config.sigma^2)) .* cos(config.k * (y - config.centre));
        case 'diff_gauss'
            select_fcn = @(x, y, z) -(y - config.centre) .* exp(-((y - config.centre).^2) / (2 * config.sigma^2)) / (config.sigma^2);
        case 'step'
            select_fcn = @(x, y, z) 0.5 * (y > config.centre) - 0.5 * (y <= config.centre);
        case 'linear'
            select_fcn = @(x, y, z) (y - config.centre) / config.sigma;
        otherwise
            error('Unknown select_fcn_type: %s', config.select_fcn_type);
    end
end
...
function analyze_correlation(config)
    ...
    Corr_vs_K_AllDiameters(base_dir, diameters, config.mdl, config.stim, sigma_fixed, k_values, smooth_coeff);
end
...
function Corr_vs_K_AllDiameters(base_dir, diameters, mdl, stim, sigma_fixed, k_values, smooth_coeff)
