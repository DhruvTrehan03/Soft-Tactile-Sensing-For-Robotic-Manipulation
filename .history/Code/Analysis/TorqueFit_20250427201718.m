...
if sigma_corr
    % Define parameters
    base_dir = 'SavedVariables\TorqueFitting_Fixed';
...
if k_corr
    ...
    for i = 1:length(select_fcns)
        fprintf('Processing with select_fcn #%d...\n', i);
        fit_and_visualize(base_dir, diameters, mdl, stim, select_fcns{i}, k_values, smooth_coeff);
    end
end
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff)
    ...
    for d = 1:length(diameters)
        ...
        torque_file = fullfile(base_dir, sprintf('Torque_All_%d.mat',diameter));

        % Load torque values
        torque_data = load(torque_file);
        torque_values =torque_data.pks';
    ...
    xlabel('Sigma');
    ylabel('Correlation');
    title(sprintf('Correlation vs Sigma for All Diameters: Select Function %s', func2str(select_fcn)));
    % Add a vertical line for each diameter's optimal sigma
    for d = 1:length(diameters)
        [~, idx] = max(avg_corrs{d});
        optimal_sigma = sigma_values(idx);
        xline(optimal_sigma, 'Color', colors(d, :), 'LineWidth', 2, 'DisplayName', sprintf('%s Diameter (Optimal)', diameters{d}));
    end
    legend('Location', 'best');
    grid on;
