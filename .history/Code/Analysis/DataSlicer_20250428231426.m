...
% Prompt user for file name and diameter
right_file = input('Enter the full path to the right file: ', 's');
truth_file = input('Enter the full path to the truth file: ', 's');
d = input('Enter the diameter (in mm): ');
...
for i = 1:length(closest_values)
    ...
    data_diff = data - hom;
    save(sprintf("Code\\SavedVariables\\Diameter_Slices\\%dmm_Data_%i", d, i), "data_diff", "mean_torque");
