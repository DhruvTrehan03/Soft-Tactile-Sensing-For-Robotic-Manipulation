...
if sigma_corr
    ...
    select_fcns = {
        @(x, y, z) exp(-(y - 1.8).^2 / (2 * z^2)) .* cos(k_const * (y - 1.8)),  % Modulated Gaussian
        @(x, y, z) -(y - 1.8) .* exp(-((y - 1.8).^2) / (2 * z^2)) / (z^2),     % Difference of Gaussians
        @(x, y, z) 0.5 * (y > 1.8) - 0.5 * (y <= 1.8),                         % Step function
        @(x, y, z) (y - 1.8) / z                                               % Linear function
    };
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff)
    ...
    % and visualizes the goodness of fit.
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            for j = 1:length(sigma_values)
                ...
                plain.fwd_model.stimulation = stim;
                press = plain;
                press.elem_data = 1 + elem_select(press.fwd_model, @(x, y, z) select_fcn(x, y, sigma));
                press.fwd_model.stimulation = stim;
        ...
        % Create a figure for this diameter
        figure;
        smoothing_factor = 0.2; % Default smoothing factor
        smoothed_corr = smoothdata(avg_corr, 'SmoothingFactor', smoothing_factor);

        % Plot initial smoothed correlation
        plot_handle = plot(sigma_values, smoothed_corr, 'Color', colors(d, :), 'LineWidth', 2, ...
            'DisplayName', sprintf('%s Diameter', diameter));
        hold on;
        xlabel('Sigma');
        ylabel('Correlation');
        ...
        uicontrol('Style', 'slider', ...
            'Min', 0.1, 'Max', 1, 'Value', smoothing_factor, ...
            'Position', [20, 20, 200, 20], ...
            'Callback', @(src, event) update_smoothing(src.Value, avg_corr, sigma_values, plot_handle));

        % Add label for select_fcn number
        uicontrol('Style', 'text', ...
            'Position', [240, 20, 100, 20], ...
            'String', sprintf('SelectFcn #%d', d));
    end
end

function update_smoothing(new_smoothing_factor, avg_corr, sigma_values, plot_handle)
    % UPDATE_SMOOTHING: Updates the plot with a new smoothing factor.
    smoothed_corr = smoothdata(avg_corr, 'SmoothingFactor', new_smoothing_factor);
    set(plot_handle, 'YData', smoothed_corr); % Update the plot
end
