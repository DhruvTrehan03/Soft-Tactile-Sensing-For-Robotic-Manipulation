...
function sim_data_matrix = GenerateDataMatrix(mdl, stim, plain,plain_data, select_fcn, k_values, sigma_values)
    ...
    update_interval = 100; % Update progress every 10 iterations
    % Loop through k and sigma values
    for i = 1:num_k
        k = k_values(i);
        for j = 1:num_sigma
            % Update progress bar less frequently
            if mod(((i-1)*num_sigma+j), update_interval) == 0
                waitbar(((i-1)*num_sigma+j)/(num_k*num_sigma), progress, ...
                    sprintf('Generating Simulated Data Matrix: k=%.2f, σ=%.2f', k, sigma_values(j)));
            end
            % Reduce frequency of disp
            if mod(((i-1)*num_sigma+j), update_interval) == 0
                disp(sprintf('Processing k=%.2f, sigma=%.2f', k, sigma_values(j)));
            end
