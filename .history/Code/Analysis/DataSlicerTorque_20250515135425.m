...
EIT = EIT(:,~all(EIT==0));
EIT_O = EIT;
EIT = smoothdata(EIT', 'gaussian', 45)'; % Smooth data across all times for each channel
...
% Cluster torque values into 10 groups
num_clusters = 10;
[cluster_indices, cluster_centers] = kmeans(pks, num_clusters);

% Replace torque values with their corresponding cluster centers
grouped_torque_values = cluster_centers(cluster_indices);

% Ensure EIT_Time and grouped torque times are in the same range
...
% Find the closest EIT times to the grouped torque times
...
for i = 1:length(closest_indices)
    ...
    torque_value = grouped_torque_values(i); % Corresponding grouped torque value
    save(sprintf("SavedVariables\\Torque_Slices_Grouped\\Data_Slice_%i", i), "data_diff", "torque_value");
