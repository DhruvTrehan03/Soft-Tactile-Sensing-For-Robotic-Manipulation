...
% Plot an example data slice
example_index = 1; % Choose the first slice as an example
example_slice = EIT(closest_indices(example_index), :); % Extract the data slice

figure();
plot(1:length(example_slice), example_slice, 'b');
title(sprintf("Example Data Slice %d", example_index));
xlabel("Channel Index");
ylabel("EIT Signal");
legend("Data Slice");

% Subplot for homogeneous data and peak slices
figure();
subplot(2, 1, 1);
...
subplot(2, 1, 2);
hold on;
num_slices_to_plot = min(3, length(closest_indices)); % Plot up to 3 slices
for i = 1:num_slices_to_plot
    slice = EIT(closest_indices(i), :);
    plot(1:length(slice), slice, 'DisplayName', sprintf("Slice %d", i));
end
title("Peak Slices");
xlabel("Channel Index");
ylabel("EIT Signal");
