...
k_values = linspace(2.9, 3.25, 500); % Do in sections
sigma_values = linspace(1, 1.55, 500);
% k_values = linspace(3.25, 3.6, 500); % Do in sections
% sigma_values = linspace(1.55, 2.2, 500);
...
function sim_data_matrix = GenerateDataMatrix(mdl, stim, plain,plain_data, select_fcn, k_values, sigma_values)
    ...
    save_file = 'sim_data_matrix_focused.mat'; 
    if exist(save_file, 'file')
        delete(save_file); % Remove existing file to avoid conflicts
    end
    save(save_file, 'k_values', 'sigma_values', '-v7.3'); % Save metadata first
    ...
    for i = 1:num_k
        ...
        for j = 1:num_sigma
            ...
            % Save the current slice incrementally
            slice = squeeze(sim_data_matrix(i, j, :));
            save(save_file, 'slice', '-append');
        end
        disp(sprintf('Completed run %i in time: %.2f seconds', i, toc));
    end

    % Save the matrix to a file
    save('sim_data_matrix_500_1.mat', 'sim_data_matrix', 'k_values', 'sigma_values');
    disp(sprintf('Press data matrix saved to sim_data_matrix_%i.mat',length(k_values)));
end
