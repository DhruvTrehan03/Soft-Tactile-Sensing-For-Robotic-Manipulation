...
if sigma_corr
    ...
    diameters = {'10mm', '20mm', '40mm'};
    centre = 1.8;
    sigma_values = linspace(0.1, 1.2, 20);
    k_values = linspace(1, 10, 10);
    smooth_coeff = 50;

    % Create figure and UI controls
    fig = figure('Name', 'Correlation Analysis', 'NumberTitle', 'off');
    uicontrol('Style', 'checkbox', 'String', 'Vary Sigma', ...
        'Position', [20, 80, 100, 20], 'Value', 1, ...
        'Callback', @(src, ~) update_plot());
    uicontrol('Style', 'checkbox', 'String', 'Vary k', ...
        'Position', [20, 50, 100, 20], 'Value', 0, ...
        'Callback', @(src, ~) update_plot());
    uicontrol('Style', 'slider', 'Min', min(k_values), 'Max', max(k_values), ...
        'Value', k_values(1), 'Position', [150, 50, 200, 20], ...
        'Callback', @(src, ~) update_plot(), 'Tag', 'k_slider');
    uicontrol('Style', 'slider', 'Min', min(sigma_values), 'Max', max(sigma_values), ...
        'Value', sigma_values(1), 'Position', [150, 80, 200, 20], ...
        'Callback', @(src, ~) update_plot(), 'Tag', 'sigma_slider');

    % Nested function to update the plot
    function update_plot()
        % Get checkbox and slider values
        vary_sigma = findobj(fig, 'String', 'Vary Sigma').Value;
        vary_k = findobj(fig, 'String', 'Vary k').Value;
        k_const = findobj(fig, 'Tag', 'k_slider').Value;
        sigma_const = findobj(fig, 'Tag', 'sigma_slider').Value;

        % Clear the figure
        clf(fig);

        % Plot based on user selection
        if vary_sigma && vary_k
            % Plot heatmap for varying both sigma and k
            correlations = compute_correlations(base_dir, diameters, mdl, stim, sigma_values, k_values, smooth_coeff);
            imagesc(k_values, sigma_values, correlations);
            colorbar;
            xlabel('k');
            ylabel('Sigma');
            title('Correlation Heatmap (Varying Sigma and k)');
        elseif vary_sigma
            % Plot correlation vs sigma for constant k
            correlations = compute_correlations(base_dir, diameters, mdl, stim, sigma_values, k_const, smooth_coeff);
            plot(sigma_values, correlations, 'LineWidth', 2);
            xlabel('Sigma');
            ylabel('Correlation');
            title(sprintf('Correlation vs Sigma (k = %.2f)', k_const));
        elseif vary_k
            % Plot correlation vs k for constant sigma
            correlations = compute_correlations(base_dir, diameters, mdl, stim, sigma_const, k_values, smooth_coeff);
            plot(k_values, correlations, 'LineWidth', 2);
            xlabel('k');
            ylabel('Correlation');
            title(sprintf('Correlation vs k (Sigma = %.2f)', sigma_const));
        end
    end
...
function correlations = compute_correlations(base_dir, diameters, mdl, stim, sigma_values, k_values, smooth_coeff)
    % Initialize storage
    if numel(sigma_values) > 1 && numel(k_values) > 1
        correlations = zeros(length(sigma_values), length(k_values));
    else
        correlations = zeros(max(length(sigma_values), length(k_values)), 1);
    end
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(sigma_values)
            for j = 1:length(k_values)
                ...
                select_fcn = @(x, y, z) exp(-(y - 1.8).^2 / (2 * sigma^2)) .* cos(k * (y - 1.8));

                % Compute correlation
                for f = 1:length(files)
                    data_diff = load(fullfile(files(f).folder, files(f).name)).data_diff';
                    plain = mk_image(mdl, 1, 'Hi');
                    plain.fwd_model.stimulation = stim;
                    press = plain;
                    press.elem_data = 1 + elem_select(press.fwd_model, select_fcn);
                    ...
                    correlations(i, j) = correlations(i, j) + envelope_correlation(data_diff, sim_diff, smooth_coeff);
                end
                correlations(i, j) = correlations(i, j) / length(files); % Average correlation
            end
        end
    end
end
