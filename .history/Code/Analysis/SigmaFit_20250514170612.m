...
function SFit(base_dir, diameters, mdl, stim,plain, plain_data, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    heatmap_data = zeros(length(k_values), length(sigma_values));
        ...
        for i = 1:length(files)
            ...
            for k_idx = 1:length(k_values)
                for sigma_idx = 1:length(sigma_values)
                    ...
                    heatmap_data(k_idx, sigma_idx) = max(cross_corr); % Take the maximum correlation
                end
            end
            ...
            % Generate heatmap
            figure;
            imagesc(sigma_values, k_values, heatmap_data);
            colorbar;
            xlabel('Sigma');
            ylabel('k');
            title(sprintf('Cross-Correlation Heatmap for Diameter: %s, File: %d', diameter, i));
        end
    end
