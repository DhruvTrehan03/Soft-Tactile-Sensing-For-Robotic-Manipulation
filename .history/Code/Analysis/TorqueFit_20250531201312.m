...
% data = load('sigma_corr_matrix.mat');
% sigma_values = data.existing_sigma_values(:);
% correlation_matrix = smoothdata(data.correlation_matrix(:,:), 'gaussian', 2);

% % Compute the gradient
% gradient_matrix = gradient(correlation_matrix, sigma_values, 2);

% % Plot the gradient
% plot(sigma_values, correlation_matrix);
% title('Gradient of Smoothed Correlation Matrix');
% xlabel('Sigma');
% ylabel('Gradient');
% legend('Diameter 10mm', 'Diameter 20mm', 'Diameter 30mm', 'Diameter 40mm');
% grid on;
...
function env = calculate_envelope(data, smooth_coeff)
    ...
    [env_ext, ~] = envelope(data_ext, 20, 'peak');
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            % figure();
            % hold on;
            % plot(data_diff); % Plot data_diff for reference
            % plot(calculate_envelope(data_diff, smooth_coeff)); % Plot envelope of data_diff
            % hold off;
