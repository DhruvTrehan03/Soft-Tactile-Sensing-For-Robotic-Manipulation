...
scatter3(maxima_matrix(:, 2), maxima_matrix(:, 3), maxima_matrix(:, 1), 50, maxima_matrix(:, 4), 'filled'); 
xlabel('k');
ylabel('Sigma');
zlabel('Diameter');
...
torque_sorted = maxima_matrix(:, 4);
% Display correlation between torque and k
disp('Correlation between Diameter and k:');
disp(corr(torque_sorted, k_sorted));

%plot for k vs torque with polynomial fits
figure;
plot(k_sorted, torque_sorted, 'o', 'DisplayName', 'Data');
hold on;
p1_k = polyfit(k_sorted, torque_sorted, 1);
fitted_torque_p1_k = polyval(p1_k, k_sorted);
mse_p1_k = mean((torque_sorted - fitted_torque_p1_k).^2);
nmse_p1_k = mse_p1_k / var(torque_sorted);
disp('Diameter vs k:');
disp(['Order 1 nMSE: ', num2str(nmse_p1_k)]);
plot(k_sorted, fitted_torque_p1_k, '-r', 'DisplayName', ...
