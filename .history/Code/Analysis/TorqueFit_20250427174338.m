...
if sigma_corr
    ...
    select_fcns = {
        ...
        @(y,s)(y>centre-k_fixed/2).*(y<centre+k_fixed/2).*(y-centre)/s, %linear with k
        @(y, s) (y <= centre) .* exp(-(y - centre).^2 / (2 * s^2)) + (y > centre) .* exp(-(y - centre).^2 / (2 * s^2)) .* cos(k_fixed * (y - centre)) % Modulated Gaussian with step
