    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            for j = 1:length(new_sigma_values)
                ...
                persistent fem_fig fem_ax;
                if isempty(fem_fig) || ~isvalid(fem_fig)
                    fem_fig = figure('Name', 'FEM Plot');
                    fem_ax = axes(fem_fig);
                end
                figure(fem_fig);
                cla(fem_ax); % Clear axes
                show_fem(press);
                drawnow;
...
function [corr_score, env1, env2] = envelope_correlation(data1, data2, smooth_coeff)
    ...
    persistent env_fig env_ax1 env_ax2;
    if isempty(env_fig) || ~isvalid(env_fig)
        env_fig = figure('Name', 'Envelope and Data Plot');
        env_ax1 = axes(env_fig);
        hold(env_ax1, 'on');
        env_ax2(1) = plot(env_ax1, NaN, 'r', 'DisplayName', 'Normalized Data1');
        env_ax2(2) = plot(env_ax1, NaN, 'b', 'DisplayName', 'Normalized Data2');
        env_ax2(3) = plot(env_ax1, NaN, 'r--', 'DisplayName', 'Envelope1');
        env_ax2(4) = plot(env_ax1, NaN, 'b--', 'DisplayName', 'Envelope2');
        legend(env_ax1, 'show');
    end
    figure(env_fig);
    set(env_ax2(1), 'YData', normalize(data1));
    set(env_ax2(2), 'YData', normalize(data2));
    set(env_ax2(3), 'YData', env1);
    set(env_ax2(4), 'YData', env2);
    drawnow;
    pause(1);
    % Compute correlation
