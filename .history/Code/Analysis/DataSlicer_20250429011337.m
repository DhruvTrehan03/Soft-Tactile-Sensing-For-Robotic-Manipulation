...
% Process the data
EIT = smoothdata(Left_Data(30:172,2:end), 'gaussian', 7);
...
hom = mean(Left_Data(1:5, 2:end),1); % Homogeneous data for the first two rows
disp(size(hom))
disp(size(EIT(closest_values, :)))
%Plot the EIT data for each slice
figure();
hold on;
for i = 1:length(closest_values)
    plot(EIT(closest_values(i), :)-hom, 'DisplayName', sprintf("Slice %d", i));
end
