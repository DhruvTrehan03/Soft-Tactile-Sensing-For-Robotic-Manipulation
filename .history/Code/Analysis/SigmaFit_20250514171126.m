...
function SFit(base_dir, diameters, mdl, stim,plain, plain_data, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    global_max_value = -Inf; % Initialize global maximum value
    global_max_coords = [];  % Initialize global maximum coordinates
    ...
    for d = 1:length(diameters)
        ...
        % Find the local maximum for the current diameter
        [local_max_value, linear_idx] = max(average_heatmap_data(:, :, d), [], 'all', 'linear');
        [local_max_k_idx, local_max_sigma_idx] = ind2sub(size(average_heatmap_data(:, :, d)), linear_idx);

        % Update global maximum if necessary
        if local_max_value > global_max_value
            global_max_value = local_max_value;
            global_max_coords = [local_max_k_idx, local_max_sigma_idx, d];
        end
    ...
    % Highlight the global maximum on the final heatmap
    if ~isempty(global_max_coords)
        global_d = global_max_coords(3);
        global_k = k_values(global_max_coords(1));
        global_sigma = sigma_values(global_max_coords(2));

        figure;
        imagesc(sigma_values, k_values, average_heatmap_data(:, :, global_d));
        colorbar;
        hold on;
        plot(global_sigma, global_k, 'rx', 'MarkerSize', 12, 'LineWidth', 2); % Highlight global maximum
        hold off;
        xlabel('Sigma');
        ylabel('k');
        title(sprintf('Final Heatmap with Global Maximum (Diameter: %s)', diameters{global_d}));
        legend('Global Maximum', 'Location', 'best');
    end
