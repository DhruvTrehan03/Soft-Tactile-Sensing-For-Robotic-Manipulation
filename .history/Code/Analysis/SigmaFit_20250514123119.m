...
function SFit(base_dir, diameters, mdl, stim,plain, plain_data, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    env_fig = figure('Name', 'Envelope Plot', 'NumberTitle', 'off','Position', [450, 100, 320, 240]);
    fem_fig = figure('Name', 'FEM Plot', 'NumberTitle', 'off','Position', [100, 100, 320, 240]);
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            progress_bar = waitbar(0, sprintf('Processing Diameter: %s, File: %d/%d', diameter, i, length(files)),"Position", [500, 500, 600, 20]);
            ...
            for j = 1:length(new_sigma_values)
                ...
                % Compute correlation
                new_correlations(d, j) = envelope_correlation(data_diff, sim_diff, smooth_coeff,env_fig);
...
function [corr_score, env1, env2] = envelope_correlation(data1, data2, smooth_coeff,env_fig)
