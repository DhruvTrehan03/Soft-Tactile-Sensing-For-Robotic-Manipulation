...
if sigma_corr
    ...
    centre = 1.8;
    k_const = 4;
    sigma_values = linspace(0.5, 1.2, 20);
    smooth_coeff = 50;
...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff)
    ...
    for d = 1:length(diameters)
        [~, idx] = max(avg_corrs{d});
        optimal_sigma = sigma_values(idx);
        xline(optimal_sigma, 'alpha', 0.5, 'Color', colors(d, :), 'LineWidth', 2, 'DisplayName', sprintf('%s Diameter (Optimal)', diameters{d}));
    end
    legend('Location', 'best');
    grid on;
