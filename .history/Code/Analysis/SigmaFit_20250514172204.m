...
function SFit(base_dir, diameters, mdl, stim,plain, plain_data, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            progress = waitbar(0, sprintf('Processing %s (%d/%d)', diameter, i, length(files)));
            % Compute cross-correlation for each k and sigma
            for k_idx = 1:length(k_values)
                for sigma_idx = 1:length(sigma_values)
                    % Update progress bar
                    waitbar((k_idx - 1) * length(sigma_values) + sigma_idx / (length(k_values) * length(sigma_values)), progress, sprintf('Processing %s (%d/%d)', diameter, i, length(files)));
    ...
    figure;
    scatter(maxima_sigma, maxima_k, 'o');
    xlabel('Sigma');
    ylabel('k');
    title('Maxima of k vs Sigma for Each Diameter');
    xticks(sigma_values);
    grid on;
    legend(diameters, 'Location', 'best');
