...
function SFit(base_dir, diameters, mdl, stim,plain, plain_data, select_fcn, sigma_values, smooth_coeff, save_path)
    ...
    env_fig = figure('Name', 'Envelope Plot', 'NumberTitle', 'off','Position', [900, 100, 800, 600]);
    fem_fig = figure('Name', 'FEM Plot', 'NumberTitle', 'off','Position', [100, 100, 800, 600]);
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            progress_bar = waitbar(0, sprintf('Processing Diameter: %s, File: %d/%d', diameter, i, length(files)),"Position", [500, 700, 300, 50]);
