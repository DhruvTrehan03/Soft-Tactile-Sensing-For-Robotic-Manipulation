...
sim_data_matrix = GenerateDataMatrix(mdl, stim, plain, plain_data, select_fcns, k_values, sigma_values, centre);

% Visualize the sim_data_matrix
VisualizeSimDataMatrix(sim_data_matrix, k_values, sigma_values);

function sim_data_matrix = GenerateDataMatrix(mdl, stim, plain,plain_data, select_fcn, k_values, sigma_values, centre)
    ...
    num_meas = length(plain.fwd_model.stimulation(1).meas_pattern(:, 1)); % Number of measurements
...
function VisualizeSimDataMatrix(sim_data_matrix, k_values, sigma_values)
    ...
    figure;
    tiledlayout(10, 10, 'TileSpacing', 'compact', 'Padding', 'compact'); % Adjust grid size as needed

    % Loop through the grid
    for i = 1:10
        for j = 1:10
            % Get the corresponding k and sigma indices
            k_idx = round(linspace(1, length(k_values), 10));
            sigma_idx = round(linspace(1, length(sigma_values), 10));
