############################################ Notes ######################################################
# Make sure bluetooth is disconnected before running this script. (The gripper will not work if there is a bluetooth connection)
...
def setup_burt(port=30010, db_host="169.254.68.20"):
    ...
    ###################################################################
    burt.movel((pos40), acc=0.05, vel=0.05) #Move to 10mm position
   ###################################################################
...
def rotate_varying(burt, gripper, start, end, steps):
    ...
    pre_rot = burt.getj()
    #####################################################################################################################
    gripper.goTomm(41, 50, 100) #diamerter, force, speed
    ######################################################################################################################
...
def main_experiment(save_file=False, use_burt=True, use_gripper=True, name="Motion"):
    ...
    if burt and gripper:
        ...
        time.sleep(1)
        ######################################################
        for i in range(8): #Number of loops
        #######################################################
            ...
            #Action
            rotate_varying(burt,gripper, 0.09, 0.09, 1)
