...
if sigma_corr
    ...
    select_fcns = {
        @( y, s) exp(-(y - 1.8).^2 / (2 * s^2)) .* cos(k_const * (y - 1.8)),  % Modulated Gaussian
        @( y, s) -(y - 1.8) .* exp(-((y - 1.8).^2) / (2 * s^2)) / (s^2),     % Difference of Gaussians
        @( y, s) 0.5 * (y > 1.8) - 0.5 * (y <= 1.8),                         % Step function
        @(y, s) (y - 1.8) / s                                               % Linear function
    };
