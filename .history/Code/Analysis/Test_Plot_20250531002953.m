...
% grid on;
% disp(size(sigma_means));
% disp(size(torque_means));
...
figure;
sorted_sigma_means = 1./sigma_means(idx_torque);
sorted_sigma_stds = 1./sigma_stds(idx_torque);
bar(sorted_sigma_means, 'FaceColor', [0.2 0.6 0.8], 'EdgeColor', 'k', 'LineWidth', 1.5);
% Set the x-ticks to the cluster indices
hold on;
errorbar(sorted_sigma_means,sorted_sigma_stds, 'k.', 'LineWidth', 1.5);
ylim([1.8 3.1]);
...
figure;
bar(unique_diameters, sigma_means_diameter.^2, 'FaceColor', [0.2 0.6 0.8], 'EdgeColor', 'k', 'LineWidth', 1.5);
hold on;
errorbar(unique_diameters, sigma_means_diameter.^2, sigma_stds_diameter, 'k.', 'LineWidth', 1.5);
xlabel('Diameter');
ylabel('Mean Sigma');
