...
EIT_O = EIT;
EIT = smoothdata(EIT(25:173,2:end), 'gaussian', 9);
...
closest_values = [20, 34, 48, 66, 84, 104,124,137]; % Adjusted for the EIT data range
...
for i = 1:length(closest_values)
    ...
    plot(calculate_envelope(abs(EIT(closest_values(i), :)-hom),50) * max(abs(EIT(closest_values(i), :)-hom)), 'DisplayName', sprintf("Slice %d", i));
    pause();
end
...
function env = calculate_envelope(data, smooth_coeff)
    ...
    [env_ext, ~] = envelope(data_ext, 25, 'peak');
