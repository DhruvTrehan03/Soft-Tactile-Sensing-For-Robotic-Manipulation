...
function initialize_eidors()
    ...
    create_model();
    config.stim = create_stimulation(config.mdl); % Use config.mdl
end
...
function mdl = create_model()
    ...
    elec_pos = [32, 1.1];
    elec_shape = 0.2;
    config.mdl = ng_mk_extruded_model(trunk_shape, elec_pos, elec_shape);
end

function stim = create_stimulation(mld)
...
function test_model(data_diff, sim_diff)
