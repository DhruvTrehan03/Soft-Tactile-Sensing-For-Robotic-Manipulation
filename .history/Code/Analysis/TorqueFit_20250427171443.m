...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, smooth_coeff)
    ...
    for d = 1:length(diameters)
        ...
        figure;
        smoothing_slider = uicontrol('Style', 'slider', ...
            'Min', 0.1, 'Max', 1, 'Value', 0.2, ...
            'Position', [20, 20, 200, 20], ...
            'Callback', @(src, event) update_smoothing(src.Value));

        % Add label for select_fcn number
        select_fcn_label = uicontrol('Style', 'text', ...
            'Position', [240, 20, 100, 20], ...
            'String', sprintf('SelectFcn #%d', i));

        % Update smoothing factor dynamically
        function update_smoothing(new_smoothing_factor)
            smoothed_corr = smoothdata(avg_corr, 'SmoothingFactor', new_smoothing_factor);
            % Update the plot or any other visualization here
            % ...existing code to refresh the plot...
        end
