...
% Initialize a map to group slices by torque values
torque_map = containers.Map('KeyType', 'double', 'ValueType', 'any');

% Group data slices by torque values
for i = 1:length(closest_indices)
    ...
    if isKey(torque_map, torque_value)
        torque_map(torque_value) = [torque_map(torque_value); data_diff];
    else
        torque_map(torque_value) = data_diff;
    end
end

% Average the slices for each torque value and save
keys = torque_map.keys;
for k = 1:length(keys)
    torque_value = keys{k};
    slices = torque_map(torque_value);
    avg_slice = mean(slices, 1); % Average across all slices for this torque value
    
    save(sprintf("SavedVariables\\Torque_Slices_Grouped_Simp\\Data_Slice_Torque_%.4f", torque_value), "avg_slice", "torque_value");
    disp(sprintf("Saved averaged slice for torque %.4f", torque_value));
end
