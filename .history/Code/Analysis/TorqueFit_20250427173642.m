...
if sigma_corr
    ...
    k_const = 4;
    sigma_values = linspace(0.1, 2, 20);
    smooth_coeff = 50;

    % Define different select_fcn options
    select_fcns = {
        @( s) exp(-(y - 1.8).^2 / (2 * s^2)) .* cos(k_const * (y - 1.8)),  % Modulated Gaussian
        @(  s) -(y - 1.8) .* exp(-((y - 1.8).^2) / (2 * s^2)) / (s^2),     % Difference of Gaussians
        @(  s) 0.5 * (y > 1.8) - 0.5 * (y <= 1.8),                         % Step function
        @( s) (y - 1.8) / s                                               % Linear function
    };
    ...
    for d = 1:length(diameters)
        ...
        for i = 1:length(files)
            ...
            for j = 1:length(sigma_values)
                ...
                plain.fwd_model.stimulation = stim;
                press = plain;
                press.elem_data = 1 + elem_select(press.fwd_model, @(x, y, z) select_fcn(sigma));
                press.fwd_model.stimulation = stim;
