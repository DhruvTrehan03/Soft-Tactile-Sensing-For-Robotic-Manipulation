...
function analyze_correlation()
    ...
    param_ranges.sigma = 4.333;
    sigma_values = linspace(2, 5, 10);
    smooth_coeff = 50;

    % Corr_vs_K_AllDiameters(base_dir, diameters, sigma_fixed, linspace(0.8, 1.1, 20), smooth_coeff);
...
function [best_params, best_corr, fit_models] = optimize_and_fit(select_fcn_template, param_ranges, data_diff, mdl, stim, torque_values, smooth_coeff, fit)
    % OPTIMIZE_AND_FIT: Generalized function to optimize parameters and fit to torque values.
    %
    % Inputs:
    % - select_fcn_template: Function handle template for select_fcn (e.g., @(k, sigma, y) ...).
    % - param_ranges: Struct with fields specifying parameter ranges (e.g., k_values, sigma_values).
    % - data_diff: Experimental data difference.
    % - mdl, stim: Model and stimulation patterns.
    % - torque_values: Torque values for fitting (optional).
    % - smooth_coeff: Smoothing coefficient for envelope correlation.
    % - fit: Boolean flag to fit parameters to torque values.
    %
    % Outputs:
    % - best_params: Struct with optimized parameters (e.g., best_k, best_sigma).
    % - best_corr: Maximum correlation achieved.
    % - fit_models: Fitted models for parameters vs torque (if fit is true).
    ...
    [param_grids{1:num_params}] = ndgrid(param_ranges.(param_names{1}), param_ranges.(param_names{2}));
    param_combinations = cell2mat(cellfun(@(x) x(:), param_grids, 'UniformOutput', false));

    % Loop through all parameter combinations
    for i = 1:size(param_combinations, 1)
        % Extract current parameter values
        current_params = num2cell(param_combinations(i, :));
        param_struct = cell2struct(current_params, param_names, 2);

        % Generate select_fcn using current parameters
        select_fcn = @(x, y, z) select_fcn_template(param_struct, y);
        ...
        if corr_score > best_corr
            best_corr = corr_score;
            best_params = param_struct;
        end
    end

    % Fit parameters to torque values (if applicable)
    if fit && ~isempty(torque_values)
        param_values = struct2array(best_params);
        fit_models = struct();
        for j = 1:num_params
            param_name = param_names{j};
            fit_models.(param_name) = fit(torque_values, param_values(:, j), 'poly3');
        end
    end
end

function analyze_diameters(base_dir, diameters, param_ranges, fit_flag, plot_flag, smooth_coeff)
    % ANALYZE_DIAMETERS: Comprehensive function to load data, optimize parameters, fit, and plot results.
    %
    % Inputs:
    % - base_dir: Base directory containing data for all diameters.
    % - diameters: Cell array of diameter names (e.g., {'10mm', '20mm'}).
    % - param_ranges: Struct with fields specifying parameter ranges (e.g., k_values, sigma_values).
    % - fit_flag: Boolean flag to fit parameters to torque values.
    % - plot_flag: Boolean flag to plot results.
    % - smooth_coeff: Smoothing coefficient for envelope correlation.
    %
    % Outputs:
    % - Results are saved to files and optionally plotted.

    global config;
    mdl = config.mdl;
    stim = config.stim;

    % Determine optimization mode (single or both parameters)
    optimize_k = isfield(param_ranges, 'k');
    optimize_sigma = isfield(param_ranges, 'sigma');
    ...
    for d = 1:length(diameters)
        ...
        % Use a valid field name for the struct
        valid_field_name = matlab.lang.makeValidName(['D_' diameter]);
        ...
        % Check if sigma is a single value
        is_sigma_constant = isscalar(param_ranges.sigma);

        % Initialize storage for correlations
        if optimize_k && ~is_sigma_constant
            correlations = zeros(length(files), length(param_ranges.k), length(param_ranges.sigma));
        elseif optimize_k && is_sigma_constant
            correlations = zeros(length(files), length(param_ranges.k));
        elseif optimize_sigma
            correlations = zeros(length(files), length(param_ranges.sigma));
        end
        ...
        for i = 1:length(files)
            ...
            % Optimize parameters
            [best_params, best_corr, ~] = optimize_and_fit(config.select_fcns{config.selected_fcn_idx}, ...
                param_ranges, data_diff, mdl, stim, torque_values, smooth_coeff, fit_flag);

            % Store results
            if optimize_k && ~is_sigma_constant
                correlations(i, :, :) = best_corr;
            elseif optimize_k && is_sigma_constant
                correlations(i, :) = best_corr;
            elseif optimize_sigma
                correlations(i, :) = best_corr;
            end
            all_results.(valid_field_name).best_params{i} = best_params;
        end
        ...
        if plot_flag
            if optimize_k && ~is_sigma_constant
                ...
                figure;
                imagesc(param_ranges.k, param_ranges.sigma, smoothed_corr);
                colorbar;
                xlabel('k');
                ylabel('Sigma');
            ...
            elseif optimize_k && is_sigma_constant
                ...
                figure;
                plot(param_ranges.k, smoothed_corr, 'LineWidth', 2, ...
                     'DisplayName', sprintf('%s Diameter', diameter));
                xlabel('k');
                ylabel('Correlation');
                title(sprintf('Correlation vs k for %s Diameter (Sigma = %.2f)', diameter, param_ranges.sigma));
                legend('Location', 'best');
                grid on;
            elseif optimize_sigma
                % Plot correlation vs sigma
                figure;
                plot(param_ranges.sigma, smoothed_corr, 'LineWidth', 2, ...
                     'DisplayName', sprintf('%s Diameter', diameter));
                xlabel('Sigma');
                ylabel('Correlation');
                title(sprintf('Correlation vs Sigma for %s Diameter', diameter));
                legend('Location', 'best');
                grid on;
            end
        end
    ...
    % Fit parameters to torque values (if requested)
    if fit_flag
        all_torques = [];
        all_k = [];
        all_sigma = [];
        for d = 1:length(diameters)
            diameter = diameters{d};
            for i = 1:length(all_results.(diameter).best_params)
                all_torques = [all_torques; torque_values(i)];
                if optimize_k
                    all_k = [all_k; all_results.(diameter).best_params{i}.k];
                end
                if optimize_sigma
                    all_sigma = [all_sigma; all_results.(diameter).best_params{i}.sigma];
                end
            end
        end

        % Fit k and sigma to torque values
        if optimize_k
            k_fit = fit(all_torques, all_k, 'poly3');
        end
        if optimize_sigma
            sigma_fit = fit(all_torques, all_sigma, 'poly3');
        end
        ...
        if plot_flag
            if optimize_k
                figure;
                scatter(all_torques, all_k, 'bo'); hold on;
                plot(k_fit, 'b-');
                title('Optimized k vs Torque (Polynomial Fit)');
                xlabel('Torque');
                ylabel('k');
            ...
            if optimize_sigma
                figure;
                scatter(all_torques, all_sigma, 'ro'); hold on;
                plot(sigma_fit, 'r-');
                title('Optimized Sigma vs Torque (Polynomial Fit)');
                xlabel('Torque');
                ylabel('Sigma');
