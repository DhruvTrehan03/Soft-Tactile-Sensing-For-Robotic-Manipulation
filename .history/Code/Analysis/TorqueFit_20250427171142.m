...
function fit_and_visualize(base_dir, diameters, mdl, stim, select_fcn, sigma_values, initial_smooth_coeff)
    % FIT_AND_VISUALIZE: Fits data with simulated data using a given select_fcn
    % and visualizes the goodness of fit with an interactive smoothing slider.

    colors = lines(length(diameters)); % Generate distinct colors for plots
    figure_handle = figure('Name', 'Fit and Visualize', 'NumberTitle', 'off');

    % Create a slider for smoothing factor
    uicontrol('Style', 'text', 'Position', [20, 20, 100, 20], 'String', 'Smoothing:');
    slider = uicontrol('Style', 'slider', 'Min', 1, 'Max', 100, 'Value', initial_smooth_coeff, ...
                       'Position', [130, 20, 300, 20], 'Callback', @(src, event) update_plot(src.Value));

    % Label the figure with the select_fcn used
    annotation('textbox', [0.15, 0.85, 0.7, 0.1], 'String', func2str(select_fcn), ...
               'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontSize', 10);

    % Nested function to update the plot when the slider value changes
    function update_plot(smooth_coeff)
        ...
        xlabel('Sigma');
        ylabel('Correlation');
        title('Correlation vs Sigma for Different Diameters');
        legend('Location', 'best');
        grid on;

        % Re-add the slider and label after clearing the figure
        uicontrol('Style', 'text', 'Position', [20, 20, 100, 20], 'String', 'Smoothing:');
        uicontrol('Style', 'slider', 'Min', 1, 'Max', 100, 'Value', smooth_coeff, ...
                  'Position', [130, 20, 300, 20], 'Callback', @(src, event) update_plot(src.Value));
        annotation('textbox', [0.15, 0.85, 0.7, 0.1], 'String', func2str(select_fcn), ...
                   'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontSize', 10);
    end
    ...
    % Initial plot
    update_plot(initial_smooth_coeff);
end
