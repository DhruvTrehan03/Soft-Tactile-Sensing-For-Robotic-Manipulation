...
function [best_params, best_corr, fit_models] = optimize_and_fit(select_fcn_template, param_ranges, data_diff, mdl, stim, torque_values, smooth_coeff, fit)
    ...
    for i = 1:size(param_combinations, 1)
        ...
        config.k = param_combinations(i, 1);
        config.sigma = param_combinations(i, 2);

        % Generate select_fcn using the current config
        select_fcn = @(x, y, z) select_fcn_template(x, y, z);
    
        % Generate model
        plain = mk_image(mdl, 1, 'Hi');
        plain.fwd_model.stimulation = stim;
        press = plain;
        press.elem_data = 1 + elem_select(press.fwd_model, select_fcn_template);
        press.fwd_model.stimulation = stim;
        show_fem(press)
        disp([config.k,config.sigma])
        % Compute simulated difference
        plain_data = fwd_solve(plain);
        press_data = fwd_solve(press);
        sim_diff = abs(press_data.meas - plain_data.meas) / 10;
        ...
        if corr_score > best_corr
            best_corr = corr_score;
            best_params.k = config.k;
            best_params.sigma = config.sigma;
        end
    end
...
disp(['Using select_fcn index: ', num2str(config.selected_fcn_idx)]);
disp(['k: ', num2str(config.k), ', sigma: ', num2str(config.sigma), ', centre: ', num2str(config.centre)]);
